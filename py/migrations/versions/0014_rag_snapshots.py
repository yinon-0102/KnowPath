"""Add immutable RAG snapshots alongside legacy learning sources."""

from alembic import op
import sqlalchemy as sa

revision = "0014_rag_snapshots"
down_revision = "0013_material_raw"
branch_labels = None
depends_on = None
...
def _frozen_schema():
    # Referenced historical tables are stubs, never created or modified here.
    metadata = sa.MetaData()
    sa.Table("material_versions", metadata, sa.Column("id", sa.String(36), primary_key=True))
    sa.Table("learning_spaces", metadata, sa.Column("id", sa.String(36), primary_key=True))
    return _tables(metadata)


def upgrade():
    for table in _frozen_schema():
        table.create(op.get_bind(), checkfirst=False)


def downgrade():
    for table in reversed(_frozen_schema()):
