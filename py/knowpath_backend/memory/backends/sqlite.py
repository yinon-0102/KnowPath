...
class _ConnectionMixin:
    ...
    def get_connection(cls, path: Path) -> sqlite3.Connection:
        ...
        with cls._conn_lock:
            ...
            cls._conn_users[key] = cls._conn_users.get(key, 0) + 1
    ...
    def close(self) -> None:
        with self._conn_lock:
            ...
            remaining = self._conn_users[key] - 1
            if remaining:
                self._conn_users[key] = remaining
            else:
                del self._conn_users[key]
                self._conn_cache.pop(key).close()
...
class SQLiteColdBackend(_ConnectionMixin, ColdBackend):
    ...
    def __init__(self, path: Path):
        ...
        self.conn = self.get_connection(self.path)
...
class SQLiteVectorBackend(_ConnectionMixin, VectorBackend):
    ...
    def search(
        ...
        from knowpath_backend.memory.backends.inmemory import (
            _tfidf_score,
            _tokenize as _toks,
        )
