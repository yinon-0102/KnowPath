...
from knowpath_backend.core.message import MessageRole
...
from knowpath_backend.context.engine import (
...
from knowpath_backend.memory.seed_score import (
    AUTO_PIN_THRESHOLD,
    boost_with_kg_feedback,
    evaluate_prior_score,
    should_auto_pin,
)
from knowpath_backend.memory.recall_buffer import RecallBuffer
...
class MemoryManager:
    ...
    def __init__(
        ...
        *,
        ...
        from knowpath_backend.memory.user_layer import UserLayer
        ...
        # ── 跨项目提升:台账 + 项目层写主图钩子 ──
        from knowpath_backend.memory.kg import KnowledgeGraphConflictDetector
        ...
        if (self.user_layer is not None
            ...
            from knowpath_backend.memory.promotion_ledger import PromotionLedger
    ...
    def _build_conflict_detector(
        ...
        if strength == "extreme":
            if llm is None:
                ...
                return SimilarityConflictDetector(threshold=threshold)
            # 延迟导入避免循环
            from knowpath_backend.memory.kg import KGStore, KnowledgeGraphConflictDetector
            kg_path = None
    ...
    def _build_kg_reconciler(self):
        ...
        if not isinstance(self.conflict_detector, KnowledgeGraphConflictDetector):
            return None
        from knowpath_backend.memory.kg_reconcile import KGReconciler
    ...
    def close(self) -> None:
        """Stop background work and release this manager's storage handles."""
        ...
        try:
            ...
            self._closed = True
            if self.cold.backend is not None:
                self.cold.backend.close()
            self.semantic.backend.close()
            stores = [getattr(self.conflict_detector, "store", None), self._promotion_ledger]
            if self.user_layer is not None:
                stores.append(self.user_layer.kg.store)
            for store in stores:
