...
from knowpath_backend.workspace import Workspace, WorkspaceViolation
...
class GrepTool(Tool):
    ...
    def _run_rg(self, pattern: str, base: Path, glob: str, mode: str,
        ...
        search_dir = self.ws.root
        target = self.ws.relative(base)
        cmd = ["rg", "--color", "never", "--path-separator", "/", "--with-filename"]
        if mode == "files":
