...
def test_missing_run_does_not_block_external_cleanup_or_next_job(workspace, missing_when):
    ...
    response = delete(state, first.id)
    next_response = delete(state, second.id)
    repo = state.graph_service.repository
    jobs = {event['aggregate_id']: event for event in repo.records('outbox', event_type='material.delete')}

    def remove_run():
        if hasattr(repo, 'unit_of_work'):
            with repo.transaction(), repo.unit_of_work.session() as session:
                session.execute(sql_delete(RunEventRow).where(RunEventRow.run_id == response['run_id']))
                session.execute(sql_delete(RunRow).where(RunRow.id == response['run_id']))
        else:
    ...
    class Cleaner:
        def delete(self, payload):
            calls.append(payload['material_id'])
            if len(calls) == 1:
                if missing_when == 'during_cleanup':
                    remove_run()
                raise RuntimeError('external cleanup unavailable')

    if missing_when == 'before_claim':
        remove_run()
    timestamp = datetime.now(timezone.utc)
    consumer = MaterialDeletionWorker(service(state), Cleaner(), clock=lambda: timestamp)
    assert consumer.run_once(jobs[first.id]['id'])
    assert repo.get_record('outbox', jobs[first.id]['id'])['status'] == 'pending'
    assert not consumer.run_once(jobs[first.id]['id'])
    assert consumer.run_once(jobs[second.id]['id'])
    assert state.get_run(next_response['run_id'])['status'] == 'succeeded'
    timestamp += timedelta(minutes=10)
    assert consumer.run_once(jobs[first.id]['id'])
    assert repo.get_record('outbox', jobs[first.id]['id'])['status'] == 'completed'
