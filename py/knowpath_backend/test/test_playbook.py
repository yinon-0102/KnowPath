...
from knowpath_backend.memory import (
    L0Lifecycle,
    L0Source,
    L0Type,
    MemoryConfig,
...
def test_store_closes_sqlite_connections_on_success_and_failure(tmp_path, monkeypatch):
    import sqlite3
    import pytest
    import knowpath_backend.memory.playbook.store as module
    ...
    store = PlaybookStore(tmp_path / "closed.db")
    store.count()
...
def test_manager_close_releases_sqlite_without_breaking_other_managers(tmp_path):
    import sqlite3
    import pytest

    cfg = MemoryConfig(storage_dir=tmp_path, cold_backend="sqlite", vector_backend="sqlite")
    first, second = MemoryManager(cfg), MemoryManager(cfg)
    connection = first.cold.backend.conn
    first.close()
    first.close()
    from knowpath_backend.memory.item import MemoryItem
    second.cold.add(MemoryItem(content="still available", role="user"))
    assert second.cold.count() == 1
    second.close()
    with pytest.raises(sqlite3.ProgrammingError, match="closed"):
        connection.execute("SELECT 1")
    third = MemoryManager(cfg)
    try:
        assert third.cold.count() == 1
    finally:
        third.close()


def test_sqlite_close_keeps_original_path_after_chdir(tmp_path, monkeypatch):
    from knowpath_backend.memory.backends.sqlite import SQLiteColdBackend

    first_dir, second_dir = tmp_path / "first", tmp_path / "second"
    first_dir.mkdir()
    second_dir.mkdir()
    monkeypatch.chdir(first_dir)
    first = SQLiteColdBackend(Path("memory.db"))
    monkeypatch.chdir(second_dir)
    second = SQLiteColdBackend(Path("memory.db"))
    try:
        first.close()
