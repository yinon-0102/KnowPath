...
from knowpath_backend.verify.checkers import check_one, CheckContext, CheckerRunner
from knowpath_backend.verify.spec import Check, CheckSpec
from knowpath_backend.verify.residual import residual, effective_count
from knowpath_backend.verify.convergence import ConvergenceJudge, Verdict
...
def test_command_ok_syntax_error_skips():
    ...
    chk = Check(id="c", type="command_ok",
                params={"cmd": _python_command("a=1; for x in []: pass")})
...
def test_command_ok_assertion_fail_is_false():
    ...
    chk = Check(id="c", type="command_ok", params={"cmd": _python_command("assert False")})
    assert check_one(chk, CheckContext(result="")) is False


def test_command_ok_pass_is_true():
    chk = Check(id="c", type="command_ok", params={"cmd": _python_command("assert True")})
...
def test_runner_mixed_skip_and_pass(tmp_path):
    ...
    spec = CheckSpec(task="t", checks=[
        Check(id="bad", type="command_ok",
              params={"cmd": _python_command("x=1; for i in []: pass")}, weight=10.0),
        Check(id="good", type="command_ok",
              params={"cmd": _python_command("assert 1==1")}, weight=10.0),
    ])
...
def test_spec_prompt_warns_string_checks_cannot_see_files():
    from knowpath_backend.verify.spec import _SPEC_PROMPT
