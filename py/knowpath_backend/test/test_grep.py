...
def test_python_fallback_when_no_rg(tmp_path, monkeypatch):
    import knowpath_backend.tools.builtin.grep as g
...
def test_nested_search_uses_workspace_paths_and_preserves_content(tmp_path, monkeypatch, fallback):
    import knowpath_backend.tools.builtin.grep as g
    if fallback:
        monkeypatch.setattr(g.shutil, "which", lambda _name: None)
    folder = tmp_path / "nested"
    folder.mkdir()
    content = r"match C:\\source\\file.py"
    (folder / "item.txt").write_text(content, encoding="utf-8")
    out = GrepTool(Workspace(tmp_path)).run({"pattern": "match", "path": "nested"})
    assert "nested/item.txt:1:" in out
