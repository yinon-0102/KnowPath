...
from my_agent_llms.learning.db import Base, IdempotencyRow, MaterialRow, MaterialVersionRow, SourceChunkRow, RunRow, OutboxEventRow
...
def graph_workspace(request, tmp_path):
    ...
    try:
        ...
        if engine and request.param == "mysql":
            from my_agent_llms.learning.db import GraphRevisionRow
            with engine.begin() as connection:
                ...
                connection.execute(delete(OutboxEventRow).where(OutboxEventRow.aggregate_id.in_([r.id for r in revisions])))
                ...
                versions = select(MaterialVersionRow.id).where(MaterialVersionRow.material_id == upload.material.id)
...
def test_identical_versions_report_unchanged_nodes():
    from my_agent_llms.learning.graph_reconciliation import compare_snapshots, extract_snapshot
    state = LearningState()
...
def test_snapshot_hash_and_conflicts_do_not_depend_on_database_row_order():
    ...
    from my_agent_llms.learning.graph_reconciliation import compare_snapshots, digest, extract_snapshot
    state = LearningState()
