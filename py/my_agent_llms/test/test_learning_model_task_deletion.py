...
from my_agent_llms.learning.learning_repository import TABLES
from my_agent_llms.learning.material_deletion import references
from my_agent_llms.test.test_learning_material_deletion import workspace, seed, service
from my_agent_llms.test.test_learning_model_tasks import enqueue, worker
from my_agent_llms.test.test_learning_state_persistence import FixedQuestions, FixedAnswer
...
def test_orphaned_model_event_is_settled_without_calling_provider(workspace, kind, missing):
    ...
    with repo.transaction():
        if missing == 'space':
            ...
            if hasattr(repo, 'unit_of_work'):
                from my_agent_llms.learning.db import RunRow, RunEventRow
                with repo.unit_of_work.session() as session:
...
def test_model_worker_cli_closes_retriever_and_engine(monkeypatch, fails):
    from my_agent_llms.learning import model_worker_cli as cli
    calls = []
