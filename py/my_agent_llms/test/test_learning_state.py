...
def test_assessment_finalize_updates_state_once():
    ...
    assessment = state.create_assessment(space["id"], {"kind": "diagnostic", "question_count": 1})
    question_id = assessment["questions"][0]["id"]

    state.record_attempt(assessment["id"], {"answers": [{"question_id": question_id, "answer": "A"}]})
    ...
    assert len(state.evidence_for(space["id"])) == 1
