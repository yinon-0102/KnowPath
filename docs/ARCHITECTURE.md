...
性质：领域设计与当前运行边界，非全部设计目标的验收清单。配套接口见 [API.md](API.md)。项目名称为 KnowPath，源于 Keel；当前 Web 服务使用独立学习业务模块，已接入 Agent 基座中的上下文预算、摘要和召回组件，完整工具执行循环仍独立保留。第 4、8 节记录当前代码边界；算法阈值、性能目标和评测方案仍需按实现与实验分别验证。
...
工具注册、验证和评测。Web 对话复用 ContextEngine、SummaryMemory 和 TF-IDF 检索组件；
其余通用 Agent 能力保留，但不表示已全部接入学习管线。
...
检索后由 `learning/conversation_context.py` 调用 ContextEngine 选择和去重历史上下文，
并对包含系统提示、当前问题和引用资料的最终输入估算预算。当前问题不可截断，至少保留一个
来源；超出必要输入预算时明确失败。主题关系、来源权威性、薄弱程度的联合排序仍待评测。
...
Web 记忆以已完成 SQL 消息为原始记录，按空间、范围版本和资料绑定隔离；最近五轮保留原文，
更早历史生成有来源 ID 的摘录摘要，跨会话使用可重建 TF-IDF 索引召回相关句子。
摘要与召回内容作为非可信用户数据进入模型，不自动晋升为系统指令或学习者画像。
重试保留未裁剪的检索原文，空间删除清除消息，资料删除清除相关派生记忆。
