...
覆盖三种仓储重启/幂等、并发、回滚、辅助作答、未答、连续复核、20 条窗口边界及 HTTP 严格请求校验。审计记录使用 assessment context 与 evidence context 持久化。0008_assessment_audit 补上 AssessmentRow 缺失的 context 列；旧 SQL 适配器曾过滤掉顶层审计字段，复核原因未实际保存。迁移只为旧行补空对象，不能恢复此前已丢失的原因；既有评分结果和证据链保持不变。新增连续复核跨重启测试覆盖内存、SQLite、MySQL。
